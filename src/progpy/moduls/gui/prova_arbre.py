...
from PySide6.QtWidgets import (QDialog, QApplication, QTreeWidget, QHBoxLayout, QTreeWidgetItem, QVBoxLayout,
                               QComboBox,
                               QWidget, QPushButton, QSpacerItem, QSizePolicy)
...
class Finestra(QWidget):
    def __init__(self):
        ...
        self.dades_curriculum.setStyleSheet(
            "QTreeView::item { padding: 10px }height:{QFontInfo(self.font()).pixelSize() * 2}px")
    ...
    def canvi_materia_seleccionada(self):
        ...
        id_materia_afegir = self.dades_selector_materia[valor_actual]["id_materia"]
        self.afegir_dades(id_materia_afegir)
        ...
        def presencia_subitem():
            ...
            for item in range(0, self.dades_programacio.invisibleRootItem().childCount()):
                for subitem in range(0, self.dades_programacio.topLevelItem(item).childCount()):
                    if self.dades_programacio.topLevelItem(item).child(subitem).text(
                            1) == self.dades_curriculum.currentItem().text(1):
                        return True
            return False
        ...
        if self.dades_curriculum.currentItem() is not None:
            ...
            if element_afegir.type() == 0:
                if elements_presents == [] or element_afegir.text(1) not in elements_presents[0].text(1):
                    ...
                    nou_element = QTreeWidgetItem(
                        [element_afegir.text(0), element_afegir.text(1), element_afegir.text(2)], type=0)
                    ...
                    for fill in range(0, element_afegir.childCount()):
                        nou_fill = QTreeWidgetItem([element_afegir.child(fill).text(0),
                                                    element_afegir.child(fill).text(1),
                                                    element_afegir.child(fill).text(2)], type=10)
            ...
            # Si és un sub-item, el tipus és 0:
                ...
                if presencia_subitem() is False:
                    ...
                    # el mètod clone:
                    nou_fill = QTreeWidgetItem([element_afegir.text(0), element_afegir.text(1), element_afegir.text(2)],
                                               type=10)
                    calcul_alçada_nou_fill = len(nou_fill.text(1)) / 30
                    nou_fill.setSizeHint(0, QSize(310, math.ceil(calcul_alçada_nou_fill) * 11))
                    ...
                    if comprovar_pares() is False:
                        ...
                        self.dades_programacio.findItems(item_pare.text(1), Qt.MatchExactly, column=1)[0].addChild(
                            nou_fill)
